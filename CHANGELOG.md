...
- 配方 id `aurora-glass` → `theme-aurora-glass.aurora-glass`
- 配色 id `aurora` → `theme-aurora-glass.aurora`
...
- **替换**市场身份图 `resources/icon.svg`（Type-2 彩色身份图，E6#68a）——原 `icon.svg` 只有一块渐变、**没有任何识别符**——市场里跟「统一默认彩块」几乎分不出来，等于没脸。本次保留它原有的极光渐变（3 档），前面压一块磨砂玻璃面板 + 一道镜面斜光，把「glass」说出来
...
- 配方归位 `themes/`（此前摊在仓根，与作者文档「配方只许住 themes/、摊到仓根是禁止项」相反）
...
- 图标身份分工（E6#69 三图模型）：撤 marketIcon=cover.svg——icon=resources/icon.svg（Type-2）保留为市场展示；整幅封面迁入 README 展示
